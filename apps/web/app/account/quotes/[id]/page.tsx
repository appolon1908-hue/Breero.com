...
export default function QuoteDetail() {
  ...
  if (error)
    return <ErrorState title="Quote not available" description={error.message} onRetry={retry} />;
  if (!quote) return <LoadingState label="Loading quote" />;
  return (
    <>
      <a className="account-back" href="/account/quotes">
        ← Back to quotes
      </a>
                ...
                {quote.line_items.map((line, index) => (
                  <tr key={`${line.description}-${index}`}>
                    ...
                    <td>
                      ...
                      }).format((line.quantity * line.unit_price_minor) / 100)}
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <div>
                ...
                <span>
                  {new Intl.NumberFormat("en-GB", {
                    style: "currency",
                    currency: quote.currency,
                  }).format(quote.subtotal_minor / 100)}
                </span>
              </div>
              <div>
                <span>Tax</span>
              ...
              <div>
                ...
                <strong>
                  {new Intl.NumberFormat("en-GB", {
                    style: "currency",
                    currency: quote.currency,
                  }).format(quote.total_minor / 100)}
                </strong>
              </div>
            </div>
          </Card>
        </div>
        <Card className="account-col-4 approval-panel">
          {quote.status === "PENDING" ? (
            <QuoteApproval
              quoteId={quote.id}
            ...
            <div className="approval-success">
              ...
              <p>This quote has already been decided.</p>
            </div>
          )}
        </Card>
      </div>
