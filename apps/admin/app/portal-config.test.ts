...
describe("Administration portal configuration", () => {
  it("only reads endpoints that exist in the API contract", () => {
    ...
    expect(findContractProblems(adminPortalConfig, openapi)).toEqual([]);
  });

  it("gives every blocked section a reason and a way out", () => {
    for (const section of adminPortalConfig.sections.filter((item) => !item.source)) {
  ...
  it("has unique section slugs", () => {
    const slugs = adminPortalConfig.sections.map((section) => section.slug);
