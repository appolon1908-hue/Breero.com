...
| FE-04 | MEDIUM | Frontend contract | `packages/types/`, `scripts/` | `@breero/types` was hand-maintained and `check-frontend-openapi.mjs` asserted only that paths and methods existed. No field name, type, nullability or enum value was compared, so a backend rename shipped green and failed in the browser | Types generated from `openapi.json` and committed; a drift check regenerates and compares them; type-level conformance assertions bind the hand-written contracts to the generated schema. Assertions are strict on field existence and compatible-in-either-direction on type, so safe widening such as `status: string` is permitted while a real conflict is not | `check-generated-types.mjs` plus `packages/types/src/conformance.ts` under `tsc --noEmit`. Verified in both directions: renaming the field on the frontend and simulating the rename on the API each fail the build | `00fc939` | UNAVAILABLE | FIXED |
