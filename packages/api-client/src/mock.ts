...
export interface MockScenario {
  ...
  latencyMs?: number;
  fail?: Partial<Record<keyof BreeroApi, Error>>;
}
...
export function createMockBreeroApi(scenario: MockScenario = {}): BreeroApi {
  ...
  return {
    ...
    auth: {
      ...
      changePassword: () =>
        run("auth", () => ({ message: "Password changed; active sessions revoked" })),
      verifyEmail: () => run("auth", () => ({ message: "Email verified" })),
      resendVerification: (s) =>
        run("auth", () => ({ message: "Verification sent if required" }), s),
    ...
    services: {
      ...
      detail: (id, s) =>
        run(
          "services",
          () => scenario.services?.find((item) => item.id === id) ?? missing(`service ${id}`),
          s,
        ),
      questions: (id, s) =>
        run(
          "services",
          () =>
            scenario.services?.find((item) => item.id === id)?.questions ??
            missing(`service ${id}`),
          s,
        ),
    },
    addresses: {
      validate: (_input, s) => run("addresses", () => scenario.address ?? missing("address"), s),
    },
    availability: { search: (_input, s) => run("availability", () => scenario.slots ?? [], s) },
    bookings: {
      create: (_input, _key, s) =>
        run(
          "bookings",
          () => scenario.bookingCreateResponse ?? missing("bookingCreateResponse"),
          s,
        ),
      prepareGuestPayment: (_id, _token, _key, s) =>
        run("payments", () => scenario.payments?.[0] ?? missing("payment"), s),
      guestConfirmation: (id, _token, s) =>
        run(
          "bookings",
          () => {
            const booking =
              scenario.bookings?.find((item) => item.id === id) ?? missing(`booking ${id}`);
            ...
            return {
              ...
              amount_minor: Math.round(Number(booking.total_amount) * 100),
              currency: booking.currency,
              next_action:
                booking.status === "CONFIRMED"
                  ? ("confirmed" as const)
                  : ("await_payment_confirmation" as const),
            };
          },
          s,
        ),
      mine: (_params, s) =>
        run(
          "bookings",
          () => ({
            items: scenario.bookings ?? [],
            total: scenario.bookings?.length ?? 0,
            page: 1,
            page_size: 20,
          }),
          s,
    ...
    payments: {
      createIntent: (_input, _key, s) =>
        run("payments", () => scenario.payments?.[0] ?? missing("payment"), s),
    },
    customer: {
      profile: (s) => run("customer", () => profile ?? missing("profile"), s),
      updateProfile: (input, s) =>
        run("customer", () => (profile = { ...(profile ?? missing("profile")), ...input }), s),
        ...
        run(
          ...
          () => {
            const items = (scenario.payments ?? []).map((payment) => ({
              ...
              payment_purpose: payment.payment_purpose ?? "BOOKING_DIAGNOSTIC",
      ...
      payment: (id, s) =>
        run(
          ...
          () => {
            const payment =
              scenario.payments?.find((item) => item.id === id) ?? missing(`payment ${id}`);
            return {
              id: payment.id,
              booking_id: payment.booking_id,
              quote_id: payment.quote_id ?? null,
    ...
    quotes: {
      list: (_params, s) =>
        run(
          "quotes",
          () => ({
            items: scenario.quotes ?? [],
            total: scenario.quotes?.length ?? 0,
            page: 1,
            page_size: 20,
          }),
          s,
      ...
      get: (id, s) =>
        run(
          "quotes",
          () => scenario.quotes?.find((item) => item.id === id) ?? missing(`quote ${id}`),
          s,
        ),
      decide: (id, approve, s) =>
        run(
